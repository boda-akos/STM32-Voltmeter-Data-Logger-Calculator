...
void sound(void){
    ...
    int dcval=analogRead(PA1); dcval/=186; //31V range, 3 octaves, 22 pitch
    if ((stat & 0x04)) {
    if (dcval>21) dcval=21; 
    if (dcval < 7) fr=freq[dcval]/2;                    //low octave
    if (dcval > 6 && dcval < 14) fr=freq[dcval % 7];    //mid
    if (dcval > 13 && dcval < 21) fr=freq[dcval % 7]*2; //high
    if (dcval > 20) fr=freq[dcval % 7]*4;               //over
    }
